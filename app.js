// SceneHunt frontend (formerly ClipHunt — localStorage keys keep the old prefix for compat). Data layer is REAL projects in localStorage — no mock/demo data.
...
function renderWorkspace() {
  ...
  document.title = `${CURRENT_PROJECT.title} · SceneHunt`;
...
function exportLinks() {
  ...
  lines.push("");
  lines.push(`Generated by SceneHunt on ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}`);
  ...
  for (const seg of SEGMENTS) {
    ...
    if (!any) {
      if (seg.clips === null && !seg.evidence && (seg.family === "evidence" || seg.family === "reference")) {
        lines.push("_Not searched yet — open this project in SceneHunt and press Find footage on this scene._");
      } else {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `${safeTitle || "scenehunt"}-visual-plan.md`;
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const isPricing = document.querySelector(".pricing-grid");
  if (!isWorkspace && !isDashboard && !isNewProject && !isPricing) return;
  ensureAuth().then(() => {
